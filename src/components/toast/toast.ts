...
export class NightjarToast extends LitElement {
  ...
  hide() {
    ...
    const toastElement = this.shadowRoot.querySelector<HTMLElement>('.toast');
    if (!toastElement) {
      throw Error('Toast element not found');
    }
  ...
  render() {
    ...
    return html`
      <div class="toast" toast-type=${this.type} ?is-hidden=${this.isHidden}>
        ...
        <!-- prettier-ignore -->
        <div class="message">${this
          .message}<slot name="message-content"></slot></div>
        <div
          class="svg-icon cross-icon"
          @click=${() => {
            this.hide();
            const event = new CustomEvent('close-button-clicked', {
              bubbles: true,
              composed: true
            });
